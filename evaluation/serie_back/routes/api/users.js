...
router.post('/register', upload.single("avatar"), async (req,res) => {
    ...
    connection.query(sqlVerify, [email], (err, resultat) => {
        ...
        if (resultat.length) {
            ...
            if (avatar) {
            const filePath = path.join(__dirname, '..','..', "/upload/avatars", avatar);
            ...
            const sql = "INSERT INTO users (pseudo, email, password, avatar, admin, verify) VALUES (?, ?, ?, ?, ?, ?)";
            connection.query(sql, [pseudo, email, hashedPassword, avatar, 0, 0], (err, result) => {
            if (err) throw err;
